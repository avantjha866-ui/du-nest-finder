...
-- Public: approved rows only. Column privacy on contact fields is handled by
-- explicit column projection in server code (this project's pattern).
...
CREATE POLICY "owners insert own listings" ON public.listings FOR INSERT TO authenticated
  WITH CHECK (
    owner_id IN (SELECT id FROM public.pg_owners WHERE user_id = auth.uid())
    AND status = 'pending'
  );
